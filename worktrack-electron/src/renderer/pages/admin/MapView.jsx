...
function createAvatarIcon(name, status, isLate) {
  const initials = (name || '?').split(' ').slice(0, 2).map(p => p[0]?.toUpperCase()).join('') || '?'
  const bg       = status === 'in_office' ? '#10B981' : '#3B82F6'
...
const officeIcon = L.divIcon({
  ...
  html: `<div style="
    ...
    display:flex;align-items:center;justify-content:center;
    font-size:20px;box-shadow:0 3px 12px rgba(0,0,0,0.5),0 0 0 4px rgba(79,134,247,0.12);
  ">🏢</div>`,
  iconSize:    [42, 42],
  iconAnchor:  [21, 21],
  popupAnchor: [0, -24],
...
export default function MapView() {
  ...
  const onMap      = checkins.filter(r => r.latitude && r.longitude)
  ...
  const STATS = [
    ...
    { label: 'On Map',    value: onMap.length,    color: 'text-gray-300',    bg: 'bg-white/[0.05]',  border: 'border-white/[0.08]'  },
  ]
          ...
          <MapContainer center={mapCenter} zoom={13} className="h-full w-full" style={{ background: '#0A0E1A' }}>
            ...
            {olat && olon && (
              <>
                ...
                <Marker position={[olat, olon]} icon={officeIcon}>
                  <Popup className="map-popup">
                    <div style={{ fontFamily: 'Inter, Arial, sans-serif', padding: '2px 4px' }}>
                      ...
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: 0 }}>Office · {radius}m geofence</p>
                    </div>
                  </Popup>
                </Marker>
              </>
            ...
            {/* Employee avatar markers */}
            {onMap.map((r, i) => (
              <Marker
                key={i}
                position={[r.latitude, r.longitude]}
                icon={createAvatarIcon(r.profiles?.full_name || '', r.status, r.is_late)}
              >
                <Popup>
                  <div style={{ fontFamily: 'Inter, Arial, sans-serif', padding: '4px 6px', minWidth: 150 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <div style={{
                        ...
                        background: r.status === 'in_office' ? '#10B981' : '#3B82F6',
                    ...
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 5, display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <p style={{ fontSize: 11, margin: 0 }}>
                        <span style={{ color: r.status === 'in_office' ? '#10B981' : '#3B82F6', fontWeight: 600 }}>
                          {r.status === 'in_office' ? '● In Office' : '⌂ WFH'}
                        </span>
          ...
          {tabList.length > 0 && (
            <div className="px-4 py-2 border-t border-white/[0.06] shrink-0">
              <p className="text-[10px] text-gray-600 text-center">
                {onMap.length} of {checkins.length} employees visible on map
              </p>
            </div>
          )}
        </div>
